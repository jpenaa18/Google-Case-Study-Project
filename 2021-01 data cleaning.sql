...
-- Because I'll be deleting rows, I will make a copy of our table. A csv of the file also exists
SELECT * 
INTO "202101_tripdata_copy"
FROM "202101_tripdata";

-- Looking at the different counts in ride_id. Ride id's are unique. This query yielded 5 distinct lengths with length of 16 making up almost all (all except < 100) 
SELECT COUNT(ride_id), LENGTH(ride_id) 
FROM "202101_tripdata"
GROUP BY LENGTH(ride_id);

-- Finding rows where ride_id is not equal to 16 characters in length. 72 rows
SELECT * FROM "202012_tripdata"
WHERE LENGTH(ride_id) != 16

-- Dropping rows where ride_id != 16. 72 rows dropped 
DELETE FROM "202101_tripdata" 
WHERE LENGTH(ride_id) != 16
...
SET rideable_type = CASE 
 ...
 -- Selecting data that has null values. 13309 rows
SELECT COUNT(*) FROM "202101_tripdata" 
WHERE start_station_naMe ISNULL
OR start_station_id ISNULL
OR end_station_name ISNULL
OR end_station_id ISNULL;

-- Deleting null values in data. 13309 rows deleted
DELETE FROM "202101_tripdata"
WHERE start_station_name ISNULL
OR start_station_id ISNULL
OR end_station_name ISNULL
