...
-- Finding rows where ride_id is not equal to 16 characters in length. 490 rows returned
SELECT * FROM "202106_tripdata"
WHERE LENGTH(ride_id) != 16

-- Dropping rows where ride_id != 16. 490 rows dropped
DELETE FROM "202106_tripdata" 
WHERE LENGTH(ride_id) != 16
...
-- Selecting data that has null values. 120,723 rows returned 
SELECT COUNT(*) FROM "202106_tripdata" 
WHERE start_station_name ISNULL
OR start_station_id ISNULL
OR end_station_name ISNULL
OR end_station_id ISNULL

-- Deleting null values in data. 120,723 rows dropped 
DELETE FROM "202106_tripdata"
WHERE start_station_name ISNULL
OR start_station_id ISNULL
OR end_station_name ISNULL
